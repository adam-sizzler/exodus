...
func SyncConfigProfileInboundsTx(ctx context.Context, tx DBTX, profileUUID string, configJSON json.RawMessage) (int, error) {
	...
	for _, inbound := range inbounds {
		...
		`, inbound.UUID, inbound.ProfileUUID, inbound.Tag, inbound.Type, networkVal, securityVal, portVal, inbound.RawInbound)
	}
	...
	if len(currentTags) > 0 {
		...
		`, profileUUID, currentTags)
	} else {
		batch.Queue(`DELETE FROM config_profile_inbounds WHERE profile_uuid = $1`, profileUUID)
	}
